...
int remoteSpeed = 150;  // Default speed (0-255)
String remoteCommand = "stop";  // Current remote command
...
const unsigned long LINE_LOST_TIMEOUT = 1000; // 1 second timeout
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (input.length() > 0) {
      // Check if it's a mode change command
      if (input.startsWith("mode:")) {
        ...
        if (mode >= 1 && mode <= 5) {
          ...
          if (currentMode != REMOTE_CONTROL_MODE) {
            remoteCommand = "stop"; // Stop when switching from remote mode
            rotateMotor(0, 0);
          }
        }
      }
      // Check if it's a remote control command
  ...
  switch (currentMode) {
    ...
    case REMOTE_CONTROL_MODE:
      ...
      break;
    default:
      rotateMotor(0, 0);  // Safe fallback
      break;
  }
}

void remoteControl() {
  digitalWrite(13, HIGH); // Indicate remote control mode
  
  if (remoteCommand == "forward") {
    rotateMotor(remoteSpeed, remoteSpeed);
  } 
  else if (remoteCommand == "backward") {
    rotateMotor(-remoteSpeed, -remoteSpeed);
  } 
  else if (remoteCommand == "left") {
    // Left turn - right motor forward, left motor backward or slower
    rotateMotor(remoteSpeed, -remoteSpeed/2);  // Changed this line
  } 
  else if (remoteCommand == "right") {
    // Right turn - left motor forward, right motor backward or slower
    rotateMotor(-remoteSpeed/2, remoteSpeed);  // Changed this line
  } 
  else { // "stop"
    rotateMotor(0, 0);
  }
